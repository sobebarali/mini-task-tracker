...
describe("Integration: PUT /api/tasks/:id - Invalid due date", () => {
	...
	it("should return 400 when due date is not ISO format", async () => {
		const task = await Task.create({
			title: "Task",
			status: TaskStatus.PENDING,
			owner: userId,
		...
		const response = await request(app)
			.put(`/api/tasks/${String(task._id)}`)
			.set("Authorization", `Bearer ${authToken}`)
			.send({ dueDate: "2024-01-01" }) // Not ISO datetime
			.expect(400);
